...
use crate::fs_ops::{read_json, write_json, with_lock, with_lock_result};
...
pub fn touch_heartbeat(agent_id: &str) -> Result<()> {
    let heartbeat_path = PathBuf::from(".rotd/coordination/heartbeat").join(format!("{}.beat", agent_id));
...
pub fn rotate_coordination_log() -> Result<()> {
    ...
    if log_path.exists() {
        ...
        let archive_path = PathBuf::from(".rotd/coordination")
            .join(format!("coordination-{}.log", today));
...
pub fn handle_command(cmd: CoordCommands, is_agent_mode: bool, verbose: bool) -> Result<()> {
    match cmd {
        CoordCommands::Claim { capability, skill_level, any } => {
            cmd_claim(capability, skill_level, any, is_agent_mode)
        }
        CoordCommands::Release { task_id } => {
            cmd_release(&task_id, is_agent_mode)
        }
        CoordCommands::Approve { task_id } => {
            cmd_approve(&task_id, is_agent_mode)
        }
        CoordCommands::Msg { message } => {
            cmd_msg(&message, is_agent_mode)
        }
        CoordCommands::Beat => {
            cmd_beat(is_agent_mode)
        }
        CoordCommands::CleanStale { timeout } => {
            cmd_clean_stale(timeout, is_agent_mode)
        }
        CoordCommands::Quota { add } => {
            cmd_quota(add, is_agent_mode)
        }
        CoordCommands::Ls => {
            cmd_ls(is_agent_mode, verbose)
        }
    }
}

fn cmd_claim(capability: Option<String>, skill_level: Option<String>, any: bool, is_agent_mode: bool) -> Result<()> {
    ...
    let result = with_lock_result(&lock_path, || -> Result<Option<WorkRegistryTask>> {
        ...
        let deps: DependencyMap = if deps_path.exists() {
            ...
            DependencyMap { deps: std::collections::HashMap::new() }
        };
        ...
        let task_statuses: Vec<(String, WorkStatus)> = registry.tasks.iter()
        ...
        for task in &mut registry.tasks {
            ...
            if let Some(task_deps) = deps.deps.get(&task.id) {
                let all_deps_done = task_deps.iter().all(|dep_id| {
                    task_statuses.iter().any(|(id, status)| id == dep_id && *status == WorkStatus::Done)
                });
...
fn cmd_release(task_id: &str, is_agent_mode: bool) -> Result<()> {
    ...
    with_lock(&lock_path, || {
        ...
        if !found {
            return Err(anyhow::anyhow!("Task not found or not claimed by this agent"));
        }
